                ...
                <div class="card">
                    @can('products-create')
                        <div class="card-header" style="justify-content: end;">
                            <a type="button" class="btn btn-primary" href="{{route('products-by-size.create')}}"> Add Product</a>
                        </div>
                    @endcan
                    <div class="card-body">
                        <table class="datatables-basic table" id="item_tables">
                            ...
                            @foreach($products as $key=>$product)
                                <tr id="item_id_{{$product->id}}">
                                    ...
                                    <td class="text-center">{{strip_tags($product->getTranslation('description','en'))}}</td>                                    
                                    <td class="text-center">
                                        @can('products-edit')
                                            <a type="button" class="btn btn-info waves-effect waves-float waves-light"
                                               href="{{route('products-by-size.edit',$product->id)}}">Edit</a>
                                        @endcan
                                        @can('products-delete')
                                            <a type="button" class="btn btn-danger waves-effect waves-float waves-light"
                                               ...
                                               item_url="{{route('products-by-size.destroy',$product->id)}}">Delete</a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
