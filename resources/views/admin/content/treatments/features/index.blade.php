...
@section('title', 'Treatment Features')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header" style="justify-content: end;">
                        <a type="button" class="btn btn-primary" href="{{route('treatment-features.create', ['id' => $treatment_id])}}"> Add Feature</a>
                    </div>
                    <div class="card-body">
                        <table class="datatables-basic table" id="item_tables">
                            ...
                            @foreach($features as $key=>$feature)
                                <tr id="item_id_{{$feature->id}}">
                                    <td class="text-center">{{++$key}}</td>                                    
                                    <td class="text-center">
                                        <img style="width:100px;height:100px" src="{{$feature->getFirstMediaUrl('logo')}}">
                                    </td>
                                    <td class="text-center">{{$feature->getTranslation('title','en')}}</td>
                                    <td class="text-center">
                                        <a type="button" class="btn btn-info waves-effect waves-float waves-light"
                                        href="{{route('treatment-features.edit',$feature->id)}}">Edit</a>
                                        <a type="button" class="btn btn-danger waves-effect waves-float waves-light"
                                        onclick="delete_item(this)" item_id="item_id_{{$feature->id}}"
                                        item_url="{{route('treatment-features.destroy',$feature->id)}}">Delete</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
