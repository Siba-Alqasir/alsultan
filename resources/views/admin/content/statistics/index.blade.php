                ...
                <div class="card">
                    @can('statistics-create')
                        <div class="card-header" style="justify-content: end;">
                            <a type="button" class="btn btn-primary" href="{{route('statistics.create')}}"> Add Statistic</a>
                        </div>
                    @endcan
                    <div class="card-body">
                        <table class="datatables-basic table" id="item_tables">
                            ...
                            @foreach($statistics as $key=>$statistic)
                                <tr id="item_id_{{$statistic->id}}">
                                    <td class="text-center">{{++$key}}</td>
                                    <td class="text-center">{{$statistic->getTranslation('title','en')}}</td>
                                    <td class="text-center">{{$statistic->value}}</td>
                                    <td class="text-center">
                                        @can('statistics-edit')
                                            <a type="button" class="btn btn-info waves-effect waves-float waves-light"
                                            href="{{route('statistics.edit',$statistic->id)}}">Edit</a>
                                        @endcan
                                        @can('statistics-delete')
                                            <a type="button" class="btn btn-danger waves-effect waves-float waves-light"
                                            onclick="delete_item(this)" item_id="item_id_{{$statistic->id}}"
                                            item_url="{{route('statistics.destroy',$statistic->id)}}">Delete</a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
