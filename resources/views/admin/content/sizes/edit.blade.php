                ...
                <div class="tab-content pt-1">
                    @foreach($active_lang as $key=>$locale)
                        <div class="tab-pane @if($key === 'en') active @endif" id="add-{{$key}}" role="tabpanel"
                            ...
                            <form method="POST" action="{{route('sizes.update',$size->id)}}" enctype="multipart/form-data" id="form_{{$key}}">
                                {{ method_field('PUT') }}
                                @csrf
                                    ...
                                    <div class="col-lg-12">
                                        <div class="mb-1">
                                            ...
                                            <input type="text" name="title" class="form-control"
                                                   value="{{$size->getTranslation('title',$key)}}"
                                ...
                                <div class="col-lg-12">
                                    <div class="mb-1">
                                        ...
                                        <textarea class="form-control" name="description" id="summernote" >{!! $size->getTranslation('description',$key) !!}</textarea>
                                    </div>
                                </div>
                                 @if($key === 'en')
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="mb-1">
                                                ...
                                                <input type="file" name="image" class="form-control" id="image_url"
                                                       accept="image/*" @if(!$size->getFirstMediaUrl('images')) required @endif />
                                            </div>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="mb-1">
                                                <img class="img-fluid rounded" src="{{$size->getFirstMediaUrl('images')}}" id="image_view">
                                            </div>
                                        </div>
                                    </div>
                                @endif
