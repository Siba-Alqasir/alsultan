                ...
                <div class="card">
                    @can('projects-create')
                        <div class="card-header d-flex justify-content-end gap-2">
                            <a type="button" class="btn btn-primary" href="{{route('projects.create')}}"> Add Project</a>
                        </div>
                    @endcan
                    <div class="card-body">
                        <table class="datatables-basic table" id="item_tables">
                            ...
                            @foreach($projects as $key=>$service)
                                <tr id="item_id_{{$service->id}}">
                                    <td class="text-center">{{++$key}}</td>
                                    <td class="text-center">{{$service->getTranslation('title','en')}}</td>
                                    <td class="text-center">
                                        @can('projects-edit')
                                            <a type="button" class="btn btn-info waves-effect waves-float waves-light"
                                               href="{{route('projects.edit',$service->id)}}">Edit</a>
                                        @endcan
                                        @can('projects-delete')
                                            <a type="button" class="btn btn-danger waves-effect waves-float waves-light"
                                               onclick="delete_item(this)" item_id="item_id_{{$service->id}}"
                                               item_url="{{route('projects.destroy',$service->id)}}">Delete</a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
