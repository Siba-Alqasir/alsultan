                ...
                <div class="tab-content pt-1">
                    @foreach($active_lang as $key=>$locale)
                        <div class="tab-pane @if($key === 'en') active @endif" id="add-{{$key}}" role="tabpanel"
                            ...
                            <form method="POST" action="{{route('finishes.update',$finish->id)}}" enctype="multipart/form-data" id="form_{{$key}}">
                                {{ method_field('PUT') }}
                                @csrf
                                    ...
                                    <div class="col-lg-12">
                                        <div class="mb-1">
                                            ...
                                            <input type="text" name="title" class="form-control"
                                                   value="{{$finish->getTranslation('title',$key)}}"
                                ...
                                <div class="col-lg-12">
                                    <div class="mb-1">
                                        ...
                                        <textarea class="form-control" name="description">{{strip_tags($finish->getTranslation('description',$key))}}</textarea>
                                    </div>
                                </div>
                                 @if($key === 'en')
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="mb-1">
                                                ...
                                                <input type="file" name="image" class="form-control" id="image_url"
                                                       accept="image/*" @if(!$finish->getFirstMediaUrl('images')) required @endif />
                                            </div>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="mb-1">
                                                <img class="img-fluid rounded" src="{{$finish->getFirstMediaUrl('images')}}" id="image_view">
                                            </div>
                                        </div>
                                    </div>
                                @endif
