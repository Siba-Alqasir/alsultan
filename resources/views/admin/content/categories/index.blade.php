            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header" style="justify-content: end;">
                        <a type="button" class="btn btn-primary" href="{{route('categories.create')}}"> Add Category</a>
                    </div>
                    @endcan --}}
                    <div class="card-body">
                        <table class="datatables-basic table" id="item_tables">
                            ...
                            @foreach($categories as $key=>$category)
                                <tr id="item_id_{{$category->id}}">
                                    <td class="text-center">{{++$key}}</td>
                                    <td class="text-center">{{$category->getTranslation('title','en')}}</td>
                                    <td class="text-center">
                                        ...
                                        <a type="button" class="btn btn-success waves-effect waves-float waves-light"
                                           href="{{route('products.index',['category' => $category->id])}}">Products</a>
                                        @endcan
                                        {{-- @can('categories-delete')
                                        <a type="button" class="btn btn-danger waves-effect waves-float waves-light"
                                           onclick="delete_item(this)" item_id="item_id_{{$category->id}}"
                                           item_url="{{route('categories.destroy',$category->id)}}">Delete</a>
                                        @endcan --}}
                                    </td>
                                </tr>
                            @endforeach
