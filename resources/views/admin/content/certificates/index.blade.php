                ...
                <div class="card">
                    @can('certificates-create')
                        <div class="card-header" style="justify-content: end;">
                            <a type="button" class="btn btn-primary" href="{{route('certificates.create')}}"> Add Certificate</a>
                        </div>
                    @endcan
                    <div class="card-body">
                        <table class="datatables-basic table" id="item_tables">
                            ...
                            @foreach($certificates as $key=>$certificate)
                                <tr id="item_id_{{$certificate->id}}">
                                    <td class="text-center">{{++$key}}</td>                                    
                                    <td class="text-center">
                                        <img style="width:100px;height:100px" src="{{$certificate->getFirstMediaUrl('logo')}}">
                                    </td>
                                    <td class="text-center">{{$certificate->getTranslation('title','en')}}</td>
                                    <td class="text-center">
                                        @can('certificates-edit')
                                            <a type="button" class="btn btn-info waves-effect waves-float waves-light"
                                            href="{{route('certificates.edit',$certificate->id)}}">Edit</a>
                                        @endcan
                                        @can('certificates-delete')
                                            <a type="button" class="btn btn-danger waves-effect waves-float waves-light"
                                            onclick="delete_item(this)" item_id="item_id_{{$certificate->id}}"
                                            item_url="{{route('certificates.destroy',$certificate->id)}}">Delete</a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
