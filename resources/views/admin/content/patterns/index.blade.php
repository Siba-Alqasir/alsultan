                ...
                <div class="card">
                    @can('patterns-create')
                        <div class="card-header" style="justify-content: end;">
                            <a type="button" class="btn btn-primary" href="{{route('patterns.create')}}"> Add Pattern</a>
                        </div>
                    @endcan
                    <div class="card-body">
                        <table class="datatables-basic table" id="item_tables">
                            ...
                            @foreach($patterns as $key=>$pattern)
                                <tr id="item_id_{{$pattern->id}}">
                                    <td class="text-center">{{++$key}}</td>
                                    <td class="text-center">
                                        <img style="width:100px;height:100px" src="{{$pattern->getFirstMediaUrl('images')}}">
                                    </td>
                                    <td class="text-center">{{$pattern->getTranslation('title','en')}}</td>
                                    <td class="text-center">
                                        @can('patterns-edit')
                                            <a type="button" class="btn btn-info waves-effect waves-float waves-light"
                                               href="{{route('patterns.edit',$pattern->id)}}">Edit</a>
                                        @endcan
                                        @can('patterns-delete')
                                            <a type="button" class="btn btn-danger waves-effect waves-float waves-light"
                                               onclick="delete_item(this)" item_id="item_id_{{$pattern->id}}"
                                               item_url="{{route('patterns.destroy',$pattern->id)}}">Delete</a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
